...
objoverlapthreshs = [0.00001 ] ;

for OVERLAPS = 1:1 %4:length(objoverlapthreshs)
    ...
    for  YY = 1:ay
        ...
        mpenvdir = '/Users/marq789/Documents/PROJECTS/WACCEM/MPera5envs_rev2wang/';
        ...
        MPtrackdir = strcat(rootdir,'/vorttracking_rev2wangclone/');
        MPstatsdir = strcat(rootdir,'/vortstats_rev2wangclone/');


        MPtrackname = strcat('wang_vorbpf_tracks_morevar',YYYY,'*.nc')  ;  %file name search criteria for synotpic pixel object file search

        %home tester set:
        %MPstatsf = strcat('/trackstats_final_',YYYY,'0501.0000_',YYYY,'0531.2300.nc')  ;   % name convention of synoptic object stats file
        %full set:
        MPstatsf = strcat('/wang_trackstats_final_',YYYY,'0501.0000_',YYYY,'0831.2300.nc')  ;   % name convention of synoptic object stats file
        ...
        matout = strcat(rootdir,'/matlab/',YYYY,'_vorstats_masks_zone_v7_rev2wangclone_MatchupEnvs_objoverlap',num2str(objoverlapthresh*100),'percent.mat') ;   %name/location of .mat output save file
        ...
        %%%% only reading in a PW field because other env vars wouldnt complete on
        %%%% nersc in 24 hrs.
        PWinfile = [mpenvdir,'mp_wang_tracks_era5_pw_',YRLIST(YY,:),'0501.0000_',YRLIST(YY,:),'0831.2300.nc'] ;
        meanPW_MPstats      = ncread(PWinfile,'meanPW');
        maxPW_MPstats       = ncread(PWinfile,'maxPW');
        minPW_MPstats       = ncread(PWinfile,'minPW');


%%% temporary bypass of loading these because the original env code is
%%% failing.
...
%         if(YY == 11)             %pending: 2014    %%%% 2014 times out at 24 hours wallclock! Ugh, will probably have to spli
            meanMUCAPE_MPstats = area_MPstats;          meanMUCAPE_MPstats(:) = NaN;
            maxMUCAPE_MPstats = area_MPstats;           maxMUCAPE_MPstats(:) = NaN;
            meanMUCIN_MPstats = area_MPstats;           meanMUCIN_MPstats(:) = NaN;
            minMUCIN_MPstats = area_MPstats;            minMUCIN_MPstats(:) = NaN;
            meanMULFC_MPstats = area_MPstats;           meanMULFC_MPstats(:) = NaN;
            meanMUEL_MPstats = area_MPstats;            meanMUEL_MPstats(:) = NaN;
%             meanPW_MPstats = area_MPstats;              meanPW_MPstats(:) = NaN;
%             maxPW_MPstats = area_MPstats;               maxPW_MPstats(:) = NaN;
%             minPW_MPstats = area_MPstats;               minPW_MPstats(:) = NaN;
%         else
...
%         end
        ...
        % catalog U and V values associated with MP feature - you would
        % normally do this from Zhe's env code, but it's not running in
        % time for these Wang MP tracks
        meanU600_MPstats = area_MPstats;    meanU600_MPstats(:) = NaN;
        meanV600_MPstats = area_MPstats;    meanV600_MPstats(:) = NaN;

        disp('   ')
        disp('catalog U & V values associated with MP feature')
        tic
        for t_met = 1:sa-1
            %t_met = 21;
            bt_met = basetime_MPtracks(t_met);

            masked_U600 = U600(:,:,t_met);
            masked_U600(isnan( cloudtracknumber_MPtracks(:,:,t_met) ))  = NaN;
            masked_V600 = V600(:,:,t_met);
            masked_V600(isnan( cloudtracknumber_MPtracks(:,:,t_met) ))  = NaN;
            ...
            for n = 1:length(curr_MPfeatures)
                % n = 3
                %%%%%%%%% pick up here testing if it is grabbing the right intra-MP U/V data from the current scene
                mean_maskU = mean( masked_U600(  find(cloudtracknumber_MPtracks(:,:,t_met) == curr_MPfeatures(n))  ) );  % mean of velocity at all pts in a synoptic mask at current time
                %now place this in the meanU600_MPstats array by finding the syn object basetime:
                meanU600_MPstats(  find( basetime_MPstats(:,curr_MPfeatures(n)) == bt_met) , curr_MPfeatures(n) ) = mean_maskU ;

                %%%%%%%%% pick up here testing if it is grabbing the right intra-MP U/V data from the current scene
                mean_maskV = mean( masked_V600(  find(cloudtracknumber_MPtracks(:,:,t_met) == curr_MPfeatures(n))  ) );  % mean of velocity at all pts in a synoptic mask at current time
                %now place this in the meanU600_MPstats array by finding the syn object basetime:
                meanV600_MPstats(  find( basetime_MPstats(:,curr_MPfeatures(n)) == bt_met) , curr_MPfeatures(n) ) = mean_maskV ;
            end
        end
        toc
        % convert to wind speed/direction
        meanWNDSPD600 = ( meanU600_MPstats .* meanU600_MPstats     +     meanV600_MPstats .* meanV600_MPstats  ).^0.5 ;
        meanWNDDIR600 = atan2(meanU600_MPstats,meanV600_MPstats)*(180/3.14159) + 180 ;
