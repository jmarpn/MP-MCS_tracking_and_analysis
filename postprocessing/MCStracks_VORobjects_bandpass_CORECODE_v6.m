...
for  YY = 12 : 12 %ay
    ...
    clearvars -except ay YRLIST TRACKVAR plotme latrange lonrange YY iterative_LS_recovery numiterations 


    YYYY = YRLIST(YY,:) ;

    
    rootdir = '   ' ;    %root dir location of syn object flextrkr files       
    mpenvdir = '     '; %location of environment output files

    % directory of location of MCS stats files
    MCSstatdir = '   ' ;
    % directory location of MCS track files
    MCStracksdir = '    ';
    ...
    %there were different stats file formats based on the tracking year
    ...
    %save output (.mat):
    matout = strcat(rootdir,'/matlab/',YYYY,'_vorstats_masks_zone_v6_MatchupEnvs.mat') ;   %name/location of .mat output save file
...
meanWNDDIR600(:,MASK_TOSSERS_MP) = NaN;   
  ...
  %  %  commenting because of annoying sub-function issues on nersc. can figure this out later if you want plots  
   ...
   for TT =  242:1:249  %length(basetime_MPtracks)
       ...
       ax6 = axes;
       ax7 = axes;
       linkaxes([ax1,ax2,ax3,ax4,ax5,ax6,ax7],'xy')    %p
       ...
       contour(ax2,lon_met,lat_met,tmp,[0.99:1:1000.99],'m','LineWidth',1);
       ...
       contour(ax3,lon_met,lat_met,MPmasks_masked(:,:,TT)',[-1 -0.99],'k','LineWidth',1)
       hold on
       
       contour(ax4,lon_met,lat_met,MPmasks_masked(:,:,TT)',[0.99 1],'k','LineWidth',2)
       hold on
       ...
       contour(ax5,lon_met,lat_met,MPmasks_masked_withmcs(:,:,TT)',[0.99 1],'-r','LineWidth',2)
       ...
       for p = 1:length(currLStracks)
           ...
           temp_lon = meanlon_LSstats( now, currLStracks(p));
...
%                 temp_lon = meanlon_MPstats( 1:now, currMPtracks(p));
       ...
       if(isempty(currMCS)==0)
           ...
           contourf(ax7,lon_mcs + 360, lat_mcs, permute(track,[2 1]),[currMCS(1) currMCS(end)],'FaceColor',[1 0 0])%,'LineWidth',6 )
           ...
           if(isempty(currMCS)==0)
               for p = 1:length(currMCS)
                   ...
                   text(ax7,temp_lon+360, temp_lat, num2str(currMCS(p)),'FontSize',15,'Color',[0.5 0 0] )
               end
           end
       ...
       outlab = horzcat('tracking_example.eps');
